...
class MicroGempKafkaResourceIT {
    ...
    void setup() {
        ...
        MicroGempKafkaResource kafkaResource = new MicroGempKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/micro-gemp-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/micro-gemp-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
